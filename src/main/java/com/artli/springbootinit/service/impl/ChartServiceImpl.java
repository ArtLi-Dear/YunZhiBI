...
//取到原始文件名
     ...
     */
    @Override
    public BaseResponse<BiResponse> intelGetByAiAsc(MultipartFile multipartFile, ChartgetRequest chartgetRequest, HttpServletRequest request) {
        String goal = chartgetRequest.getGoal();
        String name = chartgetRequest.getName();
        String chartType = chartgetRequest.getChartType();
        //校验
...
//取到原始文件名
        ...
        CompletableFuture.runAsync(() ->{
            Chart chartAsy = new Chart();
            chartAsy.setStatus("running");
            chartAsy.setId(chart.getId());
            boolean b = updateById(chartAsy);
            if (!b) {
                updateChartError(chart.getId(),"更新图表状态失败!");
                return;
            }
            String result = aiManager.doChat(AI_ID, userInput.toString());
            ...
            if (split.length < 3) {
                updateChartError(chart.getId(),"AI生成错误");
            }
            ...
            boolean updateChart = updateById(chart1);
            if (!updateChart) {
                updateChartError(chart.getId(),"更新图表成功状态失败");
            }
        },threadPoolExecutor);
...
//        biResponse.setGenResult(chart.getGenResult());
    ...
    private void updateChartError(Long id,String message){
        Chart chartAsy = new Chart();
        chartAsy.setStatus("failed");
        chartAsy.setId(id);
        chartAsy.setExecMessage(message);
        boolean b = updateById(chartAsy);
        if (!b) {
            log.error("更新图表失败状态失败" + id +"," + message);
        }
    }
}
